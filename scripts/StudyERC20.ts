...
let main = async () => {
    ...
    console.log('ERC20 balance:', owner.address, ethers.utils.formatEther((await instance.balanceOf(owner.address)).toString()));
    ...
    ,instance.address, ethers.utils.formatEther((await instance.totalSupply()).toString())
    );
    console.log('ERC20 balance:'
    ,instance.address, ethers.utils.formatEther((await instance.balanceOf(instance.address)).toString())
    );
    console.log('ERC20 balance:'
    ,instanceInc.address, ethers.utils.formatEther((await instance.balanceOf(instanceInc.address)).toString())
    );
    console.log('ERC20 balance:'
    ,owner.address, ethers.utils.formatEther((await instance.balanceOf(owner.address)).toString())
    );
